...
class BiddingTypeController extends Controller
{
    ...
    {
        $query = BiddingType::getFilteredBiddingTypes($request->all());

        $size = $request->query('size', 10);
        $page = $request->query('page', 1);

        $biddingTypes = $query->paginate($size, ['*'], 'page', $page);

        $transformedBiddingTypes = $biddingTypes->map(function ($biddingType) {
            return [
                'id' => $biddingType->id,
        ...
        return response()->json([
            ...
            'data' => [
                'data' => $transformedBiddingTypes,
                'total_elements' => $biddingTypes->total(),
                'total_pages' => $biddingTypes->lastPage(),
                'page_size' => $biddingTypes->perPage(),
                'current_page' => $biddingTypes->currentPage(),
            ],
        ], 200);
    }
    ...
    public function store(StoreBiddingTypeRequest $request)
    {
        ...
        try {
            $biddingType = BiddingType::createBiddingType($request->all());
    ...
    {
        $biddingType = BiddingType::findBiddingTypeById($id);
    ...
    public function update(UpdateBiddingTypeRequest $request, $id)
    {
        ...
        try {
            ...
            $biddingType = BiddingType::updateBiddingType($id, $data);
    ...
    public function destroy(string $id)
    {
        ...
        try {
            $biddingType = BiddingType::deleteBiddingType($id);
